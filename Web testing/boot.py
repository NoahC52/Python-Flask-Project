...
def sms_handle():
    ...
    # If the user entered a phone number, but didn't enter a body for the SMS, we return an error.
    elif body == "":
        output = "No text found within the body"
        return render_template('return_sms.html', output=output)
...
def weather_handle():
    ...
    url_lat = f"https://api.openweathermap.org/geo/1.0/zip?zip={zipcode},{country}&appid={weather_api}"
